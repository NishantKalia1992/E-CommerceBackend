    ...
    stages {
        ...
        stage('Deploy to AWS') {
            steps {
                ...
                bat '''
                ssh -i "D:\\Tools\\aws-microservices-key.pem" -o StrictHostKeyChecking=no ubuntu@54.82.85.215 "cd E-CommerceBackend && git pull origin main && docker-compose down && docker-compose pull && docker-compose up -d"
                '''
            }
        }
    }
